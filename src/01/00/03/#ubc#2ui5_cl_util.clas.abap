...
CLASS /ubc/2ui5_cl_util IMPLEMENTATION.
  ...
  METHOD filter_update_tokens.
    ...
    DATA(lt_range) = /ubc/2ui5_cl_util=>filter_get_range_t_by_token_t( result[ name = name ]-t_token ).
  ...
  METHOD json_parse.
    TRY.
        ...
        /ubc/2ui5_cl_ajson=>parse( val )->to_abap(
          EXPORTING
  ...
  METHOD json_stringify.
    TRY.
        ...
        DATA(li_ajson) = CAST /ubc/2ui5_if_ajson( /ubc/2ui5_cl_ajson=>create_empty( ) ).
...
`x, nsis, objectivec, ocaml, pascal, perl, pgsql, php, plain_text, powershell, praat, prolog, properties, protobuf, python, r, razor, rdoc, rhtml, rst, ruby, rust, sass, scad, scala, scheme, scss, sh, sjs, smarty, snippets, soy_template, space, sql,` &&
  ...
  METHOD x_check_raise.
    ...
    IF when = abap_true.
      RAISE EXCEPTION TYPE /ubc/2ui5_cx_util_error EXPORTING val = v.
    ENDIF.
