CLASS /ubc/2ui5_cl_core_app DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    DATA ms_draft TYPE /ubc/2ui5_if_types=>ty_s_get-s_draft.
    ...
    METHODS model_json_parse
      IMPORTING
        iv_view  TYPE clike
        io_model TYPE REF TO /ubc/2ui5_if_ajson.
    ...
    CLASS-METHODS all_xml_parse
      IMPORTING
        !xml          TYPE clike
      RETURNING
        VALUE(result) TYPE REF TO /ubc/2ui5_cl_core_app.
    ...
    CLASS-METHODS db_load_by_app
      IMPORTING
        app           TYPE REF TO /ubc/2ui5_if_app
      RETURNING
        VALUE(result) TYPE REF TO /ubc/2ui5_cl_core_app.
...
CLASS /ubc/2ui5_cl_core_app IMPLEMENTATION.

  METHOD all_xml_parse.

    /ubc/2ui5_cl_util=>xml_parse( EXPORTING xml = xml
  ...
  METHOD all_xml_stringify.
    ...
    TRY.
        ...
        result = /ubc/2ui5_cl_util=>xml_stringify( me ).
      ...
      CATCH cx_root INTO DATA(x2).
        TRY.
            ...
            DATA(lo_dissolver) = NEW /ubc/2ui5_cl_core_srv_diss( attri = mt_attri
                                                             app   = mo_app ).

            lo_dissolver->main( ).
            lo_dissolver->main( ).
            lo_model = NEW /ubc/2ui5_cl_core_srv_attri( attri = mt_attri
                                                    app   = mo_app ).
            lo_model->attri_before_save( ).
  ...
  METHOD db_load.
    ...
    DATA(lo_db) = NEW /ubc/2ui5_cl_core_srv_draft( ).
    DATA(ls_db) = lo_db->read_draft( id ).
    result = all_xml_parse( ls_db-data ).

    DATA(lo_model) = NEW /ubc/2ui5_cl_core_srv_attri( attri = result->mt_attri
                                                  app   = result->mo_app ).
  ...
  METHOD db_save.
    ...
    IF mo_app IS BOUND.
      CAST /ubc/2ui5_if_app( mo_app )->id_draft = ms_draft-id.
      CAST /ubc/2ui5_if_app( mo_app )->check_initialized = abap_true.
    ENDIF.
  ...
  METHOD model_json_parse.
    ...
    DATA(lo_json_mapper) = NEW /ubc/2ui5_cl_core_srv_json( ).
