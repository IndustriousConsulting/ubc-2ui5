...
CLASS ltcl_test IMPLEMENTATION.
  METHOD request_json_to_abap.
    ...
    DATA(lo_mapper) = NEW /ubc/2ui5_cl_core_srv_json( ).
    DATA(ls_result) = lo_mapper->request_json_to_abap( lv_payload ).

    DATA(ls_exp) = VALUE /ubc/2ui5_if_core_types=>ty_s_request( s_front = VALUE #( id       = `ID_NR`
                                                                               view     = `MAIN`
                                                                               origin   = `ORIGIN`
                                                                               pathname = `PATHNAME`
                                                                               search   = `SEARCH`
    ...
    DATA(lt_tree) = VALUE /ubc/2ui5_if_ajson_types=>ty_nodes_ts( ).
