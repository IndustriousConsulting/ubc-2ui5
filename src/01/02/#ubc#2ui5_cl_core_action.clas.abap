CLASS /ubc/2ui5_cl_core_action DEFINITION
  PUBLIC FINAL
  CREATE PUBLIC.

  PUBLIC SECTION.
    DATA mo_http_post TYPE REF TO /ubc/2ui5_cl_core_handler.
    DATA mo_app       TYPE REF TO /ubc/2ui5_cl_core_app.

    DATA ms_actual    TYPE /ubc/2ui5_if_core_types=>ty_s_actual.
    DATA ms_next      TYPE /ubc/2ui5_if_core_types=>ty_s_next.

    METHODS factory_system_startup
      RETURNING
        VALUE(result) TYPE REF TO /ubc/2ui5_cl_core_action.
    ...
    METHODS constructor
      IMPORTING
        val TYPE REF TO /ubc/2ui5_cl_core_handler.

  PROTECTED SECTION.
    METHODS prepare_app_stack
      IMPORTING
        val           TYPE /ubc/2ui5_if_core_types=>ty_s_next-o_app_leave
      RETURNING
        VALUE(result) TYPE REF TO /ubc/2ui5_cl_core_action.
...
CLASS /ubc/2ui5_cl_core_action IMPLEMENTATION.
  ...
  METHOD factory_by_frontend.
    ...
    ELSE.
      result->mo_app = /ubc/2ui5_cl_core_app=>db_load( mo_http_post->ms_request-s_front-id ).
    ENDIF.

    result->mo_app->ms_draft-id      = /ubc/2ui5_cl_util=>uuid_get_c32( ).
    ...
    TRY.
        ...
        IF mo_http_post->ms_request-s_control-app_start_draft IS NOT INITIAL.
          TRY.
              ...
              DATA(lo_app) = /ubc/2ui5_cl_core_app=>db_load( mo_http_post->ms_request-s_control-app_start_draft ).
        ...
        DATA(li_app) = CAST /ubc/2ui5_if_app( result->mo_app->mo_app ).
  ...
  METHOD factory_stack_leave.
    ...
    " check for new app?
    TRY.
        DATA(lo_draft) = NEW /ubc/2ui5_cl_core_srv_draft( ).
  ...
  METHOD factory_system_startup.
    ...
    result->mo_app->ms_draft-id          = /ubc/2ui5_cl_util=>uuid_get_c32( ).
    result->ms_actual-check_on_navigated = abap_true.
    result->mo_app->mo_app               = /ubc/2ui5_cl_app_startup=>factory( ).
  ...
  METHOD prepare_app_stack.
    ...
    IF val->id_draft IS INITIAL.
      val->id_draft = /ubc/2ui5_cl_util=>uuid_get_c32( ).
...
*                                 ELSE ms_next-o_app_leave->id_draft ).
    ...
    result = NEW #( mo_http_post ).
    TRY.
        result->mo_app = /ubc/2ui5_cl_core_app=>db_load_by_app( val ).
      CATCH cx_root.
