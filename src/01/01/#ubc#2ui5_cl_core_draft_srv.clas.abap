CLASS /ubc/2ui5_cl_core_draft_srv DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    METHODS create
      IMPORTING
        !draft     TYPE /ubc/2ui5_if_types=>ty_s_draft
...
CLASS /ubc/2ui5_cl_core_draft_srv IMPLEMENTATION.


  METHOD cleanup.

    DATA(lv_four_hours_ago) = /ubc/2ui5_cl_util=>time_substract_seconds(
        time    = /ubc/2ui5_cl_util=>time_get_timestampl( )
        seconds = 60 * 60 * 4 ).

    DELETE FROM /ubc/2ui5_t_01 WHERE timestampl < @lv_four_hours_ago.
    COMMIT WORK.
  ...
  METHOD create.
    ...
    DATA(ls_db) = VALUE /ubc/2ui5_if_core_types=>ty_s_db(
        id                = draft-id
