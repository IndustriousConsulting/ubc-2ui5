CLASS /ubc/2ui5_cl_util_db DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS /ubc/2ui5_cl_util_db IMPLEMENTATION.



  METHOD delete_by_handle.

    DELETE FROM /ubc/2ui5_t_91
        WHERE
           uname = @uname
            AND handle = @handle
            AND handle2 = @handle2
  ...
  METHOD load_by_handle.
    ...
    DATA lt_db TYPE STANDARD TABLE OF /ubc/2ui5_t_91 WITH EMPTY KEY.
  ...
  METHOD save.
    ...
    DATA(ls_db) = VALUE /ubc/2ui5_t_91(
        uname   = uname
        handle  = handle
        handle2 = handle2
        handle3 = handle3
        data    = /ubc/2ui5_cl_util=>xml_stringify( data ) ).

    TRY.
        ls_db-id = lt_db[ 1 ]-id.
      CATCH cx_root.
        ls_db-id = /ubc/2ui5_cl_util=>uuid_get_c32( ).
    ENDTRY.

    MODIFY /ubc/2ui5_t_91 FROM @ls_db.
    ASSERT sy-subrc = 0.
