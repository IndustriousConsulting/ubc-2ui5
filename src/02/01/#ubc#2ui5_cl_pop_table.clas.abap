CLASS /ubc/2ui5_cl_pop_table DEFINITION
  PUBLIC FINAL
  CREATE PROTECTED.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS factory
      ...
      RETURNING
        VALUE(r_result) TYPE REF TO /ubc/2ui5_cl_pop_table.
...
CLASS /ubc/2ui5_cl_pop_table IMPLEMENTATION.
  METHOD display.
    ...
    DATA(popup) = /ubc/2ui5_cl_xml_view=>factory_popup( )->dialog( afterclose = client->_event( 'BUTTON_CONFIRM' )
                                                               stretch    = abap_true
                                                               title      = title
*                                                               icon       = 'sap-icon://edit'
          )->content( ).
